...
export default function ReadingModeToggle({ showLabel = false }: { showLabel?: boolean }) {
  ...
  return (
    <button
      ...
      className={
        showLabel
          ? "flex items-center gap-2 px-4 py-2 border border-border-light dark:border-border-dark hover:border-accent dark:hover:border-accent rounded-full transition-colors text-sm font-bold text-text-secondary hover:text-text-primary dark:hover:text-text-primary"
